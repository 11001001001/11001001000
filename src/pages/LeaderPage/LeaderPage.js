...
const LeaderPage = () => {
  ...
  const checkAndUpdateBalance = async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/api/ch${process.env.REACT_APP_AHHHF_IOE_PESDX}eck-user/${userId}/`);
      ...
      if (data.message === "User found") {
        ...
        if (apiBalance < balance) {
          try {
            const updateResponse = await fetch(`${process.env.REACT_APP_API_BASE_URL}/api/update-bala${process.env.REACT_APP_AHHHF_IOE_PESDX}nce/${userId}/`, {
              method: 'PUT',
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      try {
        ...
        const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/api/li${process.env.REACT_APP_AHHHF_IOE_PESDX}st-user/`);
        const data = await response.json();
        setUsers(data);
        setIsChecked(false);
      } catch (error) {
  ...
  useEffect(() => {
    fetch(`${process.env.REACT_APP_API_BASE_URL}/api/user-r${process.env.REACT_APP_AHHHF_IOE_PESDX}ank/?user_id=${userId}`)
        ...
        <div className="leader-scroll">
          ...
          <div className='top-leader'>
            <div className="leader-header">
              ...
              <div className='header-low'>{`🏆Top 100 ${getCurrentMonth()} league🏆`}</div>
            </div>
